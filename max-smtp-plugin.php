...
Description: Use multiple SMTP email accounts, cycle through your SMTP's maximum send limits, queue failed emails, and filter any annoying emails being sent out from your WordPress blog/website.
Version: 1.1.3
...
*/
	...
	class Max_SMTP_Plugin {
		...
		public static function maxsmtp_init_actions(){
			...
			add_action( 'max_smtp_clear_sent_queue', [ __CLASS__, 'maxsmtp_clear_sent_queue' ], 10, 0 );
		...
		public static function maxsmtp_deactivate_plugin(){
			...
			if( wp_next_scheduled( 'max_smtp_clear_sent_queue' ) ) {
				wp_clear_scheduled_hook( 'max_smtp_clear_sent_queue' );
			}
		}
		...
		public static function maxsmtp_uninstall_plugin(){
			...
			if( $delete_option ){
				$options	= [
							...
							'max_smtp_cron_clear_sent',
							'max_smtp_pause_status',
							'max_smtp_pause_message',
							'max_smtp_subject_filter',
							'max_smtp_address_filter',
							'max_smtp_send_filter_save',
		...
		private static function maxsmtp_schedule_cron() {
			...
			if( !wp_next_scheduled( 'max_smtp_clear_sent_queue' ) ){
				wp_schedule_event( time(), 'daily', 'max_smtp_clear_sent_queue' );
			}
		}
		...
		public static function maxsmtp_resend_no_pause( $where = 'send_status = 0' ){
			$email_sent	= 0;
			$queuq_limit	= get_option( 'max_smtp_queue_limit', 0 );
			if( $queuq_limit == 0 ){
				$queuq_limit	= 10;
		...
		public static function maxsmtp_clear_sent_queue() {
			global $wpdb;
			$mails	= $wpdb->get_results( 'SELECT id FROM ' . $wpdb->base_prefix . 'maxsmtp_queue WHERE mail_status = "Sent"', ARRAY_A );
			foreach( $mails as $mail ){
				$wpdb->delete( "{$wpdb->prefix}maxsmtp_queue", [ 'id' => $id ], [ '%d' ] );
			}
			update_option( 'max_smtp_cron_clear_sent', date( 'Y-m-d h:i:s A', strtotime( current_time( 'mysql' ) ) ) );
		}
