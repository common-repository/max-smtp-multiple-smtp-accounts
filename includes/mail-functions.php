<?php
	...
	class Max_SMTP_Mail_Functions {
		...
		public static function maxsmtp_set_smtp_phpmailer( $phpmailer ){
			...
			/**
			* Version 1.1.2 Upgrade Filter Php Mailer Body or Subject
			*/

			$phpmailer->Subject	= apply_filters( 'maxsmtp_filter_mail_subject', $phpmailer->Subject );
			$phpmailer->Body	= apply_filters( 'maxsmtp_filter_mail_body', $phpmailer->Body );
			...
			*/
			...
			/**
			* Version 1.1.1 Upgrade Subject and To Filter
			*/
			if( ! wp_cache_get( 'max_smtp_resending', 'max_smtp_cache' ) ){
				$found		= 0;
				$subject_filter	= get_option( 'max_smtp_subject_filter' );
				if( !empty( $subject_filter ) && is_array( $subject_filter ) && isset( $phpmailer->Subject ) && !empty( $phpmailer->Subject ) ){
					@preg_replace( $subject_filter, '', $phpmailer->Subject, -1, $found );
					if( $found ){
						$save_filter	= get_option( 'max_smtp_send_filter_save' );
						if( $save_filter ){
							$save_filter_data	= wp_cache_get( 'max_smtp_mail_cache', 'max_smtp_cache' );
							$email_queue	= [
											'mail_to'			=> $save_filter_data['to'],
											'mail_subject'		=> $save_filter_data['subject'],
											'mail_message'		=> $save_filter_data['message'],
											'mail_headers'		=> $save_filter_data['headers'],
											'mail_attachments'	=> $save_filter_data['attachments'],
											'mail_failed'		=> current_time( 'mysql' ),
											'mail_status'		=> 'Filtered (Subject)',
											'mail_error'		=> '',
											'send_status'		=> 1,
				...
				$address_filter	= get_option( 'max_smtp_address_filter' );
				if( !empty( $address_filter ) && is_array( $address_filter ) && !empty( $save_filter_data	= wp_cache_get( 'max_smtp_mail_cache', 'max_smtp_cache' ) ) ){
					if( isset( $save_filter_data['to'] ) && !empty( $save_filter_data['to'] ) ){
						@preg_replace( $address_filter, '', $save_filter_data['to'], -1, $found );
						if( $found ){
							$save_filter	= get_option( 'max_smtp_send_filter_save' );
							if( $save_filter ){
								$email_queue	= [
			...
			*/
			...
			if( !empty( $smtp_setting ) ){
				...
				if( ! $pause_status = get_option( 'max_smtp_pause_status' ) ){
